...
class EditParfan extends StatefulWidget {
  EditParfan({super.key, this.title, this.docid,this.list});
 final String? title;
  final String? docid;
  var list;

  @override
  State<EditParfan> createState() => _EditParfanState();
}

class _EditParfanState extends State<EditParfan> {
  GlobalKey<FormState> formState = new GlobalKey<FormState>();
  CollectionReference parfanref = FirebaseFirestore.instance.collection('parfan');
...
if(file == null)
{
    if (formdata!.validate()) {
      EasyLoading.show();
      formdata.save();
        await parfanref.doc(widget.docid).update({
          'name': title,
          'def': prais,
          'doc': doc,
  ...
  getimage() async {
    ...
    if (picked != null) {
      ...
      ref = FirebaseStorage.instance.ref('parfan').child('$nameImage');
    }
  }
